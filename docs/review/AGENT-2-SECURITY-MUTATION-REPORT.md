...
4. Phase 7 still requires contract schemas and complete implementations for protocol negotiation, revocation, cancellation, grant call quotas, grant-specific response budgets, source byte limits, input/output descriptors, and remote project-content consent. Tool descriptions and retrieved text remain data only.
5. Required red-team coverage remains incomplete for symlink race, special file, approval concurrency, current-state drift, cancellation race, crash recovery durability, inventory substitution, read-only grant reuse for apply, and credential leakage.
6. Original-fixture byte-for-byte post-test verification was not available in the Phase 9 test fixtures and must be added.
